...
import { getEmployeesWithPayslipStatus, getBranches, getDepartmentsForBranch } from "@/lib/data";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Select an employee to generate or view their payslip.</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4">
            ...
            <Select>
              <SelectTrigger><SelectValue placeholder="Payslip Status" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="generated">Generated</SelectItem>
                <SelectItem value="pending">Pending</SelectItem>
                </SelectContent>
            </Select>
            <Button className="w-full"><Search className="mr-2"/>Search</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Employee List</CardTitle>
           <CardDescription>
             Select an employee to proceed with payslip generation for the current pay period.
           </CardDescription>
        </CardHeader>
        <CardContent>
          {employees.length > 0 ? (
              ...
              <TableBody>
                {employees.map((emp) => (
                  <TableRow key={emp.id}>
                    ...
                    <TableCell>
                      <Badge variant={emp.benefitsStatus === 'Complete' ? 'default' : 'destructive'} className={emp.benefitsStatus === 'Complete' ? 'bg-blue-500' : ''}>
                        {emp.benefitsStatus}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <Badge variant={emp.payslipStatus === 'Generated' ? 'default' : 'secondary'} className={emp.payslipStatus === 'Generated' ? 'bg-green-500' : ''}>
                        {emp.payslipStatus}
                      </Badge>
                    </TableCell>
            ...
            <p className="text-muted-foreground text-center">No employees found matching your criteria.</p>
          )}
        </CardContent>
      </Card>
    </div>
