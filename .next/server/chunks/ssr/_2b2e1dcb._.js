...
const announcements = [
    {
        ...
        content: 'We are excited to announce that this years company retreat will be held at Lakeview Resort from October 10th to 12th. More details to follow!',
        posted_by_id: 'HR-001',
        status: 'Published',
        posted_at: '2024-08-15 10:00:00'
    },
    {
        id: 2,
        title: 'New Health Insurance Policy',
        content: 'Please review the updated health insurance policy documents available on the employee portal. The new policy will be effective starting next month.',
        posted_by_id: 'HR-001',
        status: 'Published',
        posted_at: '2024-08-10 14:30:00'
    },
    {
        id: 3,
        title: 'Q3 Performance Reviews',
        content: 'The Q3 performance review cycle will begin next week. Please schedule a meeting with your manager to discuss your progress.',
        posted_by_id: 'HR-001',
        status: 'Draft',
        posted_at: '2024-08-05 11:00:00'
    }
];
const attendance = [
    {
        id: 1,
        account_id: 'EMP-00123',
        attendance_date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000),
        time_in: '09:00',
        time_out: '17:00',
        status: 'Present'
    },
    {
        id: 2,
        account_id: 'EMP-00123',
        attendance_date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000),
        time_in: '09:15',
        time_out: '17:05',
        status: 'Late'
    },
    {
        id: 3,
        account_id: 'EMP-00123',
        attendance_date: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000),
        time_in: '08:58',
        time_out: '17:02',
        status: 'Present'
    },
...
const leave_requests = [
    ...
    },
    {
        ...
        reason: 'Feeling unwell, have a fever.',
        status: 'Approved'
    },
    {
        id: 3,
        account_id: 'EMP-00125',
        leave_type: 'Personal Day',
...
async function getHRDashboardData() {
    const employeeCount = accounts.filter((a)=>a.role === 'Employee' && a.status === 'Active').length;
    const pendingLeaves = leave_requests.filter((lr)=>lr.status === 'Pending').length;
    const recentActivities = leave_requests.sort((a, b)=>new Date(b.start_date) - new Date(a.start_date)).slice(0, 5).map((lr)=>{
        const employee = accounts.find((a)=>a.id === lr.account_id);
        return {
            id: lr.id,
            activity: `${employee.first_name} ${employee.last_name} requested ${lr.leave_type.toLowerCase()} leave`,
...
async function getPastAnnouncements() {
    return announcements.map((a)=>{
        const postedBy = accounts.find((acc)=>acc.id === a.posted_by_id);
        return {
            ...a,
            postedBy: `${postedBy.first_name} ${postedBy.last_name}`,
            date: new Date(a.posted_at).toISOString().split('T')[0]
        };
    }).sort((a, b)=>new Date(b.date) - new Date(a.date));
}
async function getDailyAttendanceData(date) {
    return accounts.filter((acc)=>acc.role === 'Employee' && acc.status === 'Active').map((acc)=>{
        const attendanceRecord = attendance.find((att)=>att.account_id === acc.id && new Date(att.attendance_date).toISOString().split('T')[0] === date);
        return {
            id: acc.id,
...
async function getEmployees() {
    return accounts.filter((acc)=>acc.role === 'Employee').map((acc)=>({
            id: acc.id,
            name: `${acc.first_name} ${acc.last_name}`,
            position: acc.position,
            branch: acc.branch,
...
async function getEmployeesWithPayslipStatus() {
    return accounts.filter((acc)=>acc.role === 'Employee' && acc.status === 'Active').map((acc)=>{
        const hasPayslip = payslips.some((p)=>p.account_id === acc.id);
        return {
            id: acc.id,
            name: `${acc.first_name} ${acc.last_name}`,
            position: acc.position,
...
async function getEmployeesForScheduling() {
    return accounts.filter((a)=>a.status === 'Active' && a.role === 'Employee').map((emp)=>({
            id: emp.id,
...
async function getEmployeeDashboardData(employeeId) {
    const employeeData = accounts.find((acc)=>acc.id === employeeId);
    const pubAnnouncements = announcements.filter((a)=>a.status === 'Published').map((a)=>({
            ...a,
        ...
        })).sort((a, b)=>new Date(b.posted_at) - new Date(a.posted_at)).slice(0, 5);
    return {
        employee: {
...
async function getAttendanceData(employeeId) {
    const employeeRecords = attendance.filter((a)=>a.account_id === employeeId).map((r)=>({
    ...
    const daysAttended = employeeRecords.filter((r)=>r.status === 'Present' || r.status === 'Late').length;
    const lates = employeeRecords.filter((r)=>r.status === 'Late').length;
    const absences = employeeRecords.filter((r)=>r.status === 'Absent').length;
    const totalHours = daysAttended * 8; // simplified calculation
    const availableLeaves = accounts.find((a)=>a.id === employeeId)?.available_leaves || 0;
    return {
        summary: {
            daysAttended,
            lates,
...
async function getPastLeaveRequests(employeeId) {
    return leave_requests.filter((lr)=>lr.account_id === employeeId).map((lr)=>({
            id: lr.id,
            leave_type: lr.leave_type,
            startDate: lr.start_date,
            endDate: lr.end_date,
...
async function createLeaveRequest(leaveRequest) {
    const newId = Math.max(...leave_requests.map((lr)=>lr.id), 0) + 1;
    leave_requests.push({
        id: newId,
        account_id: leaveRequest.employeeId,
...
async function getPayPeriods(employeeId) {
    return payslips.filter((p)=>p.account_id === employeeId).map((p)=>({
            ...p,
            period: `${new Date(p.period_start).toLocaleDateString()} - ${new Date(p.period_end).toLocaleDateString()}`,
            payDate: new Date(p.pay_date).toLocaleDateString('en-US', {
                month: 'long',
                day: 'numeric',
                year: 'numeric'
            })
...
async function getReportDetails(id) {
    // This is a mock, so we'll return a sample failed report for demonstration
    if (id === 'RPT-FAILED-001') {
        return {
            id: 'RPT-FAILED-001',
            type: 'Payroll',
            generatedBy: 'Olivia Chen',
            timestamp: '2024-08-18 10:30:00',
            status: 'Failed',
            details: {
                payPeriod: 'Aug 1 - Aug 15, 2024',
                employeesProcessed: 24,
                error: 'Connection to tax calculation service timed out.'
            }
        };
    }
    return null;
...
async function getPendingEmployeeApprovals() {
    return accounts.filter((acc)=>acc.status === 'Pending Approval').map((acc)=>({
            id: acc.id,
            submittedBy: 'Olivia Chen (HR)',
            employeeName: `${acc.first_name} ${acc.last_name}`,
            position: acc.position,
            submissionDate: acc.date_hired
        })).sort((a, b)=>new Date(b.submissionDate) - new Date(a.submissionDate));
}
async function approveEmployee(employeeId) {
    const account = accounts.find((acc)=>acc.id === employeeId);
    if (account) {
        account.status = 'Active';
    }
}
async function rejectEmployee(employeeId) {
    const index = accounts.findIndex((acc)=>acc.id === employeeId);
    if (index !== -1) {
...
async function addEmployee(employeeData) {
    ...
    const existing = accounts.some((acc)=>acc.email === employeeData.email);
    if (existing) {
...
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(findUserByEmail, "40940240272bf0116e2230a6f104ef339d68a9c48f", null);
(0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$webpack$2f$loaders$2f$next$2d$flight$2d$loader$2f$server$2d$reference$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["registerServerReference"])(addEmployee, "40989abdebaad175ebedaf85b69d6e64b3a5c79545", null);
}}),
...
__turbopack_context__.s({
    ...
    "40940240272bf0116e2230a6f104ef339d68a9c48f": (()=>__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["findUserByEmail"]),
    "40989abdebaad175ebedaf85b69d6e64b3a5c79545": (()=>__TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["addEmployee"]),
...
__turbopack_context__.s({
    ...
    "40940240272bf0116e2230a6f104ef339d68a9c48f": (()=>__TURBOPACK__imported__module__$5b$project$5d2f2e$next$2d$internal$2f$server$2f$app$2f$dashboard$2f$attendance$2f$page$2f$actions$2e$js__$7b$__ACTIONS_MODULE0__$3d3e$__$225b$project$5d2f$src$2f$lib$2f$data$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$2922$__$7d$__$5b$app$2d$rsc$5d$__$28$server__actions__loader$2c$__ecmascript$29$__$3c$exports$3e$__["40940240272bf0116e2230a6f104ef339d68a9c48f"]),
    "40989abdebaad175ebedaf85b69d6e64b3a5c79545": (()=>__TURBOPACK__imported__module__$5b$project$5d2f2e$next$2d$internal$2f$server$2f$app$2f$dashboard$2f$attendance$2f$page$2f$actions$2e$js__$7b$__ACTIONS_MODULE0__$3d3e$__$225b$project$5d2f$src$2f$lib$2f$data$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$2922$__$7d$__$5b$app$2d$rsc$5d$__$28$server__actions__loader$2c$__ecmascript$29$__$3c$exports$3e$__["40989abdebaad175ebedaf85b69d6e64b3a5c79545"]),
...
//# sourceMappingURL=_2b2e1dcb._.js.map